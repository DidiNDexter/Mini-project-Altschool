...
resource "aws_lb_listener" "didi_alb_listner" {
  load_balancer_arn = aws_lb.didi_alb.arn
  port              = "80"
  protocol          = "HTTP"

  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.didi_alb_tg.arn
  }
}

# create a target group for the loadbalancer
resource "aws_lb_target_group" "didi_alb_tg" {
  name     = "didialbtg"
  port     = 80
  protocol = "HTTP"
  vpc_id   = var.vpc_id
